...
import type { StudentDetailResponse } from '@/types/api'
...
const StudentDetailPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await teacherApi.getStudentDetail(id)
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 text-[var(--text-sm)] text-[var(--text-muted)]">
            ...
            <Badge color="primary">{translateRank(ratings.rank_title)}</Badge>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card padding="md" hoverable={false}>
          ...
          <p className="text-[var(--text-2xl)] font-bold text-[var(--text)]">{ratings.game_rating}</p>
          <p className="text-[var(--text-xs)] text-[var(--text-sub)] mt-1">
            {game_stats.wins}胜 {game_stats.losses}负 {game_stats.draws}和
          </p>
        </Card>
        {/* Puzzle rating */}
        <Card padding="md" hoverable={false}>
          <p className="text-[var(--text-xs)] text-[var(--text-muted)] mb-1">谜题分数</p>
          <p className="text-[var(--text-2xl)] font-bold text-[var(--text)]">{ratings.puzzle_rating}</p>
          <p className="text-[var(--text-xs)] text-[var(--text-sub)] mt-1">
            正确率 {Math.round(puzzle_stats.accuracy * 100)}% ({puzzle_stats.correct_count}/{puzzle_stats.total_attempts})
          </p>
        </Card>
        {/* Streak */}
        <Card padding="md" hoverable={false}>
          <p className="text-[var(--text-xs)] text-[var(--text-muted)] mb-1">连续学习</p>
          <p className="text-[var(--text-2xl)] font-bold text-[var(--text)]">{streak.current_login_streak} 天</p>
          <p className="text-[var(--text-xs)] text-[var(--text-sub)] mt-1">
            最高 {streak.max_login_streak} 天 / 训练连续 {streak.current_train_streak} 天
          </p>
        </Card>
      </div>

      {/* Game stats & recent games */}
      <Card padding="lg" hoverable={false}>
        ...
        <div className="flex items-center gap-6 mb-4 text-[var(--text-sm)]">
          <span className="text-[var(--text-sub)]">
            共 <strong className="text-[var(--text)]">{game_stats.total_games}</strong> 局
          </span>
          <span className="text-[var(--text-sub)]">
            胜率 <strong className="text-[var(--text)]">{Math.round(game_stats.win_rate * 100)}%</strong>
          </span>
        </div>
        ...
        {game_stats.recent_games.length > 0 ? (
          <div className="space-y-2">
            ...
            {game_stats.recent_games.map((g) => {
              const r = resultLabel(g.result)
              return (
                <div
                  key={g.id}
                  className="flex items-center gap-3 py-2 px-3 rounded-[var(--radius-xs)] bg-[var(--bg)]"
                >
      ...
      <Card padding="lg" hoverable={false}>
        ...
        <div className="flex items-center gap-6 text-[var(--text-sm)]">
          <span className="text-[var(--text-sub)]">
            共做 <strong className="text-[var(--text)]">{puzzle_stats.total_attempts}</strong> 题
          </span>
          <span className="text-[var(--text-sub)]">
            正确 <strong className="text-[var(--text)]">{puzzle_stats.correct_count}</strong> 题
          </span>
          <span className="text-[var(--text-sub)]">
            正确率 <strong className="text-[var(--text)]">{Math.round(puzzle_stats.accuracy * 100)}%</strong>
          </span>
          <span className="text-[var(--text-sub)]">
            当前连对 <strong className="text-[var(--text)]">{puzzle_stats.current_streak}</strong> 题
          </span>
        </div>
      </Card>
      ...
      <Card padding="lg" hoverable={false}>
        ...
        <div className="flex items-center gap-4 mb-4 text-[var(--text-sm)]">
          <span className="text-[var(--text-sub)]">
            已完成 <strong className="text-[var(--text)]">{course_stats.completed_lessons}</strong>/{course_stats.total_lessons} 课时
          </span>
          <span className="text-[var(--text-sub)]">
            完成率 <strong className="text-[var(--text)]">{Math.round(course_stats.completion_rate * 100)}%</strong>
          </span>
        </div>

        {course_stats.courses.length > 0 ? (
          <div className="space-y-4">
            {course_stats.courses.map((c) => (
              <div key={c.course_id}>
