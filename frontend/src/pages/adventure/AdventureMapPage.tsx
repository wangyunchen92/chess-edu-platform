...
import type { RegionItem, ChallengeItem } from '@/types/api'
...
const THEME_CONFIG: Record<string, { bg: string; border: string; glow: string }> = {
  green: {
...
// Challenge type to emoji mapping
...
const AdventureMapPage: React.FC = () => {
    ...
    adventureApi.getAdventureMap()
      .then((res) => {
        ...
        if (payload?.regions && Array.isArray(payload.regions)) {
          ...
          setCurrentRegion(payload.current_region ?? 'meadow')
          setCurrentRating(payload.current_rating ?? 300)
        }
      })
      .catch((err) => {
        console.error('[AdventureMapPage] Failed to load adventure map:', err)
      })
  ...
  const handleRegionClick = useCallback(async (region: RegionItem) => {
    ...
    try {
      ...
      // If detail fetch fails, show region with no challenges
              ...
              >
                ...
                {isLocked && (
                  <div className="absolute top-3 right-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[var(--text-muted)]"
                    ...
                    <span>{'\uD83D\uDD12'}</span>
                    <span>Rating {region.rating_range[0]} 解锁</span>
                  </div>
                )}
                ...
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[var(--text-xs)] text-[var(--text-muted)] mt-0.5">
                      Rating {region.rating_range[0]} - {region.rating_range[1] >= 9999 ? '\u221E' : region.rating_range[1]}
                    </p>
                  </div>
                </div>
                ...
                {selectedRegionDetail?.region.id === region.id && selectedRegionDetail.challenges.length > 0 && (
